...
credentials, and any TLS reference.

#### Scenario: Single reachable seed

- **WHEN** a cluster is registered with one seed URL that resolves to a live broker
- **THEN** the cluster is persisted with that node manageable and any pair member
  discovered from topology

#### Scenario: Multiple seeds for an internal-hostname cluster

- **WHEN** a cluster is registered with two seed URLs, each reachable, whose
  brokers advertise internal connector hostnames to each other
...
- **THEN** registration fails with the classified connection error and nothing is
  persisted
...
that cluster and SHALL NOT attempt any change on the broker itself.
