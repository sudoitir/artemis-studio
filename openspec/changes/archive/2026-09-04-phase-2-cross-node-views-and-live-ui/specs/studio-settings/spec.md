...
intervals, the per-node management-call ceiling, and the metric retention window
— and SHALL use the stored value when present, falling back to the packaged
configuration default otherwise. A changed setting SHALL take effect without a
restart, by the next relevant scheduled run.
...
- **WHEN** an operator submits a non-positive interval or ceiling
