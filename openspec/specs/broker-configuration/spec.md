# broker-configuration Specification
...
#### Scenario: A fragment merges into the declaration by default

- **WHEN** a cluster already declares `orders.#` with twelve keys and an operator imports a fragment declaring `#` with four keys and `orders.#` with one changed key
- **THEN** the preview offers *merge* (the default) and *replace*, and under merge the result declares both matches, `orders.#` keeping its other eleven keys with the one key updated

#### Scenario: Bare items are accepted as a snippet writes them

- **WHEN** an operator imports a `<security-setting>` and an `<address-setting>` without their `<…-settings>` wrapper, beside a `<broker-plugins>` block
- **THEN** both settings are recognised and only `broker-plugins` is listed as unsupported
...
- **WHEN** a security setting declares roles for a permission type the broker accepts but does not report back through its management read (`view` and `edit` on the versions measured)
- **THEN** the roles are sent, the plan, the verification and the drift evaluation compare only the reported types, and the editor states that those two cannot be verified from Studio
