...
intervals, the per-node management-call ceiling, the metric retention window,
the bulk-operation safety cap, the event-history retention window, and the
event write-buffer size — and SHALL use the stored value when present, falling
back to the packaged configuration default otherwise. A changed setting SHALL
take effect without a restart, by the next relevant scheduled run or the next
request that reads it. The bulk safety cap SHALL default to 1000 and the
event-history retention window SHALL default to 72 hours.
