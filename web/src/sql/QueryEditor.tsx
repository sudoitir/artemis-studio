...
const highlight = HighlightStyle.define([
  ...
  { tag: [tags.string, tags.special(tags.string)], color: 'var(--as-code-string)' },
  { tag: [tags.number, tags.bool, tags.null], color: 'var(--as-code-number)' },
  { tag: [tags.comment, tags.lineComment, tags.blockComment], color: 'var(--as-code-comment)' },
]);
...
const theme = EditorView.theme({
  ...
  '.cm-completionDetail': { color: 'var(--as-text-dimmed)', fontStyle: 'normal' },
});
...
function completions(queues: string[]) {
  ...
  const functionOptions = FUNCTIONS.map((f) => ({ label: `${f}(`, type: 'function', detail: 'function' }));
  const sourceOptions = [
  ...
  return (context: CompletionContext): CompletionResult | null => {
    ...
    if (from) {
      ...
      return {
        ...
        options: [
          ...
          ...queues.map((q) => ({ label: `"${q}"`, type: 'class', detail: 'queue' })),
        ],
        validFor: /^["\w.*#$-]*$/,
      };
    }
    ...
    return {
      ...
      options: [
        ...columnOptions,
        ...functionOptions,
        { label: 'props.', type: 'property', detail: 'an application property, by name' },
      ],
      validFor: /^[\w.]*$/,
    };
  };
...
export function QueryEditor({
  ...
  useEffect(() => {
    ...
    const extensions: Extension[] = [
      ...
      autocompletion({ override: [(c) => completions(latest.current.queues)(c)] }),
      ...
      EditorView.contentAttributes.of({ 'aria-labelledby': labelId, role: 'textbox' }),
      theme,
