...
| [0081](0081-demand-driven-client-sampling.md) | Client activity is sampled only while a cluster's flow is observed: a lease row, the cluster lock, one listProducers + listConsumers POST per node, per-member deltas into a shared disposable cache |
