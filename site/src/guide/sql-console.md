...
| `FROM "Q"` | The index when every queue it names is captured, the live brokers otherwise — a queue that is only sampled is read live. The plan says which it picked. |
